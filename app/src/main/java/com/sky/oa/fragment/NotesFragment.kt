 ...
 * @Description: 笔记模块
 */
class NotesFragment : BaseMFragment<FragmentNotesBinding, NotesViewModel>() {
...
//        开始加载数据
    ...
    override fun loadDatas() {
        viewModel.loadNotes(requireContext().assets, "Documents/笔记")
    }
                ...
                viewModel.uiState.collect { state ->
                    when (state) {
                        ...
                        is UiState.Success -> {
                            ...
                            val keys: Set<String> = state.datas.keys
                            for (key in keys) {
                                binding.tabs.addTab(
